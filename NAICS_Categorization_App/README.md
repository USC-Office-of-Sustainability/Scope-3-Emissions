...
1. Pre-process (deduplication by line-item) your training dataset (for training a new model) and/or your test datasets (for predicting NAICS codes) to decrease the processing time. We include suggestions in the code scripts below. You can use either the Rscript or the Python script for pre-processing. No need to use both. 
